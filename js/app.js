        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4">
            <div className="glass-effect p-8 rounded-3xl shadow-2xl w-full max-w-md">
                ...
                <div className="mt-4 text-center">
                    ...
                    <button
                        onClick={() => {
                            if (confirm('🚨 คำเตือน: ระบบจะ "ล้างข้อมูลการเชื่อมต่อในเครื่องนี้"\n(ข้อมูลบน Cloud จะไม่หาย แต่ต้อง Login ใหม่)\n\nทำการ Reset หรือไม่?')) {
                                localStorage.clear();
                    ...
                    >
                        ล้างข้อมูลระบุตัวตน (Factory Reset)
                    </button>
                </div>
            </div>
        </div>
...
function App() {
    ...
    useEffect(() => {
        // Initialize AI Coach
        aiCoach.current = new window.AICoach();
        audioAnalyzer.current = new window.AudioAnalyzer();
    }, []);
    ...
    async function startRecording() {
        try {
            ...
            if (audioAnalyzer.current) {
                audioAnalyzer.current.start(stream);
            }
